...
<head>
    ...
    <link href="/resource/css/home.less?v=${v}" rel="stylesheet/less" type="text/css">
    <script src="/website/less/less-3.0.0.min.js" type="text/javascript"></script>
</head>

<body>
	<div id="app" class="doc-app" v-loading.fullscreen.lock="loading" element-loading-text="拼命加载中...">
	<div class="doc-top">
    	<#if icon??>
        <div class="doc-icon">
        	<img class="doc-icon-img" src="${icon}" onclick="location.href='/home.html';">
        </div>
    	</#if>
    ...
    <div class="doc-body">
        ...
        <div class="doc-left">
            <div id="doc-menus">
                <el-menu class="el-menu-vertical-demo" theme="light" default-active="${p}">
                <#list menus as menu>
                    <el-submenu index="${menu.id}">
                        <template slot="title">${menu.describle}</template>
                        <#if menu.childs??>
                        	<#list menu.childs as child>
                            <el-menu-item index="${child.id}">
                                <a href="${child.pid}/${child.id}.html" target="doc-frame-name">${child.describle}</a>
                            </el-menu-item>
                        	</#list>
                        </#if>
                    </el-submenu>
...
<script type="text/javascript">
	// 如果网速不给力导致加载太慢时，会让页面先出现一个“拼命加载中...”的提示， 加载完成后再显示文档内容。
    new Vue({
        el: '#app',
        data: function () {
            return {
